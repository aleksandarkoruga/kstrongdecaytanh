...
struct KStrongDecayTanh : public Unit {
...
static void KStrongDecayTanh_next(KStrongDecayTanh *unit, int inNumSamples);
static void KStrongDecayTanh_Ctor(KStrongDecayTanh* unit);
static void KStrongDecayTanh_Dtor(KStrongDecayTanh* unit);


void KStrongDecayTanh_Ctor(KStrongDecayTanh* unit) {
    SETCALC(KStrongDecayTanh_next);
    ...
    if (unit->buf == NULL) {
        // Avoid retaining KStrongDecayTanh_next as the calculation function.
        SETCALC(ft->fClearUnitOutputs);
        ClearUnitOutputs(unit, 1);

        if(unit->mWorld->mVerbosity > -2) {
            Print("Failed to allocate memory for KStrongDecayTanh ugen.\n");
        }
    ...
    KStrongDecayTanh_next(unit, 1);
}

// this must be named PluginName_Dtor.
void KStrongDecayTanh_Dtor(KStrongDecayTanh* unit) {
    // Free the memory.
    RTFree(unit->mWorld, unit->buf);
}

void KStrongDecayTanh_next(KStrongDecayTanh *unit, int inNumSamples)
{
    ...
    float c_all=std::sinf(0.5*(omega-omega*pFrac))/std::sinf(0.5*(omega+omega*pFrac));
      ...
      //read phases
       ...
       //delayed samples 
        ...
        buf[writephase] = 10e5*std::tanh((in[i]+(unit->allpass))*10e-5) ;
